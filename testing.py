...
def test_block_identity_equality():
    ...
    assert id1 == id2  # однаковий вміст
    assert id1 != id3  # різний вміст
    assert id1 != "Stone"  # різні типи блоків
...
def test_building_block_composition():
    block1 = BuildingBlock(
        BlockIdentity("Glass", "Ornamental"),
        Physics(0.3, 0.3),
        Visuals(True)
    )
    ...
    block3 = BuildingBlock(
        BlockIdentity("Stone", "Structural"),
        Physics(1.5, 6.0),
        Visuals(False)
    )
...
def test_sorting_logic():
    ...
    blocks.sort(key=lambda b: (b.physics.hardness, -b.physics.blast_resistance)) # таке ж сортування, як у main4.py
